...
import { deleteAuditLog } from "@/app/actions/audit";
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">AuditLogs</h2>
        <Link href="/admin/audit/create">
          <Button className="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-2">
            <Plus size={18} /> Add AuditLog
          </Button>
        </Link>
      </div>
      <Card>
        <CardHeader className="pb-4">
          <CardTitle>Manage AuditLogs</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable 
            columns={columns} 
            ...
            actions={(item) => (
              <>
                <Link href={`/admin/audit/${item.id}/edit`}>
