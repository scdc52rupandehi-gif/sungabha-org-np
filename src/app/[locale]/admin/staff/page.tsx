...
import { getStaff, deleteStaff } from "@/app/actions/staff";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Staff Members</h2>
          <p className="text-zinc-500 dark:text-zinc-400">Manage your organization's staff profiles.</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          {loading ? (
            <div className="p-8 text-center text-zinc-500">Loading staff...</div>
          ) : staff.length === 0 ? (
            <div className="p-8 text-center text-zinc-500">No staff members found. Add one to get started.</div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    ...
                    <th className="py-3 px-4 font-medium text-sm">Position</th>
                ...
                <tbody className="divide-y divide-border">
                  {staff.map((member) => (
                    <tr key={member.id} className="hover:bg-muted/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-zinc-500">{member.position}</td>
